...
def draw_support_triangle(pos,size) :
    ...
    triangle.setData(x=x, y=y, pen=pg.mkPen(color='black',width = 1.5), fillLevel=0, fillBrush=(None))
    return triangle
def draw_support_line(pos,pos_size) :
    ...
    line.setData(x=x, y=y, pen=pg.mkPen(color='black',width = 1.5), fillLevel=0, fillBrush=(None))
    return line     
def draw_beam(lenght,size) : #ใช้อันนี้ในการสร้าง คาน
        x = [0,lenght,lenght,0,0]
        y = [0,0,size,size,0]
        plot_beam = pg.PlotDataItem()
        plot_beam.setData(x=x, y=y, pen=pg.mkPen(color='black',width = 2.0,fillLevel = -1,fillBrush=(255, 0, 0, 100)))        
        return plot_beam
...
def draw_distribute_load(span,size) :
    ...
    distribute.setData(x=x, y=y, pen=pg.mkPen(color='darkgreen',width = 1.5))        
    return distribute
def draw_dim_line(span) :
    ...
    dimline.setData(x=x, y=y, pen=pg.mkPen(color='black',width = 1.5))        
    return dimline
def draw_vertical_dim_line(pos,lenght) :
    ...
    ver_dimline.setData(x=x, y=y, pen=pg.mkPen(color='black',width = 1.5))        
    return ver_dimline
def dim_text(span) :
    offset = span[0]
...
def draw_support(obj,pos,node) : #ใช้อันนี้ในการสร้างซัพพอร์ต
    ...
    text_item = pg.TextItem(text=node, color='black', anchor=(0.5, 0.5))
...
def draw_support_dim(obj,span) :
    ...
    text_item = pg.TextItem(text=lenght, color='black', anchor=(0.5, 0.5))
    text_item.setPos(mid_span, -0.9)    
    obj.addItem(text_item)
def plot_bm(obj,beam,Lenght,query_point) :
    text_item = pg.TextItem(text='B.M.D (T.M)', color='black', anchor=(0.5, 0.5))
    text_item.setPos(0, -2)    
    x = [0,0]
    y = [-2.5,-3.5]
    obj.plot(x=x, y=y, pen=pg.mkPen(color='black',width = 1.5))
    ...
    maxY = abs(max(y,key=abs))
    y = [i/maxY for i in y]
    y = [i-3 for i in y]
    obj.plot(x, y, fillLevel=-3, fillBrush='lightblue', pen=pg.mkPen(color='blue',width = 1.5)) 
    x = [0,Lenght]
    y = [-3,-3]
    obj.plot(x=x, y=y, pen=pg.mkPen(color='blue',width = 1.0))
    ...
    for i in zipped_xy :
        ...
        if round(float(i[0]),3) in query_point and round(float(i[0]),3) not in query_check :
            val_y = float(i[1])
            print(val_y)
            text_val = pg.TextItem(text=f'{val_y:.3f}', color='blue', anchor=(0.5, 0.5))
            if -val_y >= 0 :
...
def plot_sf(obj,beam,Lenght,query_point) : 
    text_item = pg.TextItem(text='S.F.D (T)   ', color='black', anchor=(0.5, 0.5))
    ...
    y = [i/maxY for i in y]
    y = [i-5 for i in y]
    obj.plot(x, y, fillLevel=-5, fillBrush='lightgreen', pen=pg.mkPen(color='blue',width = 1.5))

    x = [0,Lenght]
    y = [-5,-5]
    obj.plot(x=x, y=y, pen=pg.mkPen(color='black',width = 1.0))
    ...
    for index , i in enumerate(zipped_xy) :
        if round(float(i[0]),3) in query_point :
            if round(float(i[0]),3) != 0 and round(float(i[0]),3) != Lenght and round(float(i[0]),3) not in query_check :
                ...
                text_val1 = pg.TextItem(text=f'{val_yleft:.3f}', color='blue', anchor=(0.5, 0.5))
                text_val1.setPos(i[0], ((val_yleft)/maxY)-4.8)

                text_val2 = pg.TextItem(text=f'{val_yright:.3f}', color='blue', anchor=(0.5, 0.5))
...
class PlotWindow(QWidget) :
    ...
    def __init__(self,filepath,DL,LL) :
        ...
        self.setWindowTitle("Beam Result")
        ...
        self.plot_widget.setFrameStyle(0)
        ...
        viewbox =self.plot_area.getViewBox()
        viewbox.enableAutoRange(axis=pg.ViewBox.XAxis)
    ...
    def UpdateValue(self) :
        ...
        painter.end()
        folder_name = "img_output"
        os.makedirs(folder_name, exist_ok=True)
        img_save_path = os.getcwd() + '\img_output'        
        #image 
        image.save(f'{img_save_path}'+f'\\{self.nobeam.currentIndex()+1}.png')
        sender = self.sender() 
...
if __name__ == "__main__" :
    ...
    window = PlotWindow(filepath = r'C:\ProjectVenv\.venv\ProjectHouse\family5-Analysed.xlsx',DL= 1.0,LL=1.0)
    window.show()
