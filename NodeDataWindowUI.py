...
class NodeWindow(QWidget):
    ...
    def __init__(self,filename):
        ...
        self.ControlNode_df = int(self.Control_Parameter_df.loc[0,'จำนวนจุดต่อ']) 
        try : 
            self.Node_df = pd.read_excel(self.filepath,sheet_name='Node_Data')
        except :
            self.Node_df = pd.DataFrame({'Node No. (int)' : [],'พิกัด X (m)(.2float)' : [],'พิกัด Y (m)  (.2float)' : [],'สถานะจุดต่อ' : []})
            for i in range(self.ControlNode_df) :
                self.Node_df.loc[i,'สถานะจุดต่อ'] = 'N'



        for i in range(self.ControlNode_df) :
            self.Node_df.loc[i,'Node No. (int)'] = int(i+1)
            
        self.Node_df = self.Node_df.fillna(0.0)
        if max(self.Node_df['Node No. (int)']) >= 13 :
            self.grid_widget.setFixedSize(QSize(380,440+(50*int(max(self.Node_df['Node No. (int)'])))))
        else :
            self.grid_widget.setFixedSize(QSize(380,440))

        for i,name in enumerate(["จุดต่อที่","พิกัด X(m)","พิกัด Y(m)","Status"]) :
        ...
        for i,val in enumerate(self.Node_df['Node No. (int)']) :
            globals()[f'Node-No_{i}'] = QLineEdit(str(self.Node_df.loc[i,'Node No. (int)']),self.grid_widget) ; globals()[f'Node-No_{i}'].setFixedSize(60,20);globals()[f'Node-No_{i}'].move(10,60+((i)*30)) ;globals()[f'Node-No_{i}'].setStyleSheet('color : yellow');globals()[f'Node-No_{i}'].setAlignment(Qt.AlignmentFlag.AlignCenter)
            globals()[f'X-CO_{i}'] = QLineEdit(str(self.Node_df.loc[i,'พิกัด X (m)(.2float)']),self.grid_widget) ; globals()[f'X-CO_{i}'].setFixedSize(60,20);globals()[f'X-CO_{i}'].move(105,60+((i)*30)) ;globals()[f'X-CO_{i}'].setStyleSheet('color : green');globals()[f'X-CO_{i}'].setAlignment(Qt.AlignmentFlag.AlignCenter)
            globals()[f'Y-CO_{i}'] = QLineEdit(str(self.Node_df.loc[i,'พิกัด Y (m)  (.2float)']),self.grid_widget); globals()[f'Y-CO_{i}'].setFixedSize(60,20);globals()[f'Y-CO_{i}'].move(210,60+((i)*30)) ; globals()[f'Y-CO_{i}'].setStyleSheet('color : green');globals()[f'Y-CO_{i}'].setAlignment(Qt.AlignmentFlag.AlignCenter)
            globals()[f'Node-Status_{i}'] = QLineEdit(str(self.Node_df.loc[i,'สถานะจุดต่อ']),self.grid_widget); globals()[f'Node-Status_{i}'].setFixedSize(60,20);globals()[f'Node-Status_{i}'].move(305,60+((i)*30)) ; globals()[f'Node-Status_{i}'].setStyleSheet('color : green');globals()[f'Node-Status_{i}'].setAlignment(Qt.AlignmentFlag.AlignCenter);globals()[f'Node-Status_{i}'].setReadOnly(True)
    ...
    def submit(self) :
        for i,value in enumerate(self.Node_df['Node No. (int)']) :
            self.Node_df.loc[i,'พิกัด X (m)(.2float)'] = float(globals()[f'X-CO_{i}'].text())
            self.Node_df.loc[i,'พิกัด Y (m)  (.2float)'] = float(globals()[f'Y-CO_{i}'].text())
    ...
    #กำหนด (200,200) คือ org(0,0) 2.5คือปรับแต่งให้ข้อความอยู่ตรงกลาง
    def VGridDraw(self) :
    ...
    def PltNode(self) :
        ...
        for i,value in enumerate(self.Node_df['Node No. (int)']) :
            print(i)
            if self.Node_df.loc[i,'พิกัด X (m)(.2float)'] != '' :
...
if __name__ == "__main__":
    ...
    window = NodeWindow(filename = r'C:\ProjectVenv\.venv\ProjectHouse\Deck.xlsx')
    window.show()
    sys.exit(app.exec())