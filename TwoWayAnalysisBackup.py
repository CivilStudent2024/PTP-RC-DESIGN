...
def TwoWay_analysed(filepath,DL,LL,fc,fy) :
    ...
    TwoWay_df = pd.DataFrame({'หมายเลขพื้น':[],'NodeI' : [] , 'NodeJ' : [] , 'NodeK' : [] , 'NodeL' : [] , 'Xlenght' : [] , 'Ylenght' : [] , 'm' : [],'Ix' : [],'Iy' : [],'Jx':[],'Jy':[],'Kx' : [],'Ky' : [],'Lx':[],'Ly':[],'DL' :[],'LL':[] })
    ...
    for index, slabtype in enumerate(Slab_Data_df['ประเภทแผ่นพื้น']) :
        if slabtype == 'พื้นหล่อในที่' :
            ...
            if 0.5 <= m <= 1.0 : #พื้นสองทาง
                ...
                new_row = pd.Series({'หมายเลขพื้น':no_slab,'NodeI' : NodeI , 'NodeJ' : NodeJ , 'NodeK' : NodeK , 'NodeL' : NodeL , 'Xlenght' : Xlenght , 'Ylenght' : Ylenght , 'm' : m,'Ix' : Ix,'Iy' : Iy,'Jx':Jx,'Jy':Jy,'Kx' : Kx,'Ky' : Ky,'Lx':Lx,'Ly':Ly,'DL':DL,'LL' : LL })
    ...
    for index, row in TwoWay_df.iterrows():
        ...
        for node,rw in TwoWay_df.iterrows() :    
            if rw['Ly'] == ys and rw['Ky'] == ys  and (rw['Lx'] <= xs1 <= rw['Kx']) and (rw['Lx'] <= xs2 <= rw['Kx'])  and  (rw['หมายเลขพื้น'] != row['หมายเลขพื้น']) :
                if rw['Ylenght'] > 0.7*Ylenght : #0.7 คือความยาวที่พิจารณาว่าเป็นคานต่อเนื่อง
                    check = 1
        if check == 1 :
        ...
        for i in [S,N,W,E] : #S,V,N,W,H,E  ตามลำดับ
            ...
            if i == W or i == E : # Ylenght
                if i[2] == 'C':
                    if Ylenght <= Xlenght : #ด้าน W,E เป็นช่วงสั้น
...
def OneWay_analysed(filepath,DL,LL,fc,fy) :
    ...
    Node_df = pd.read_excel(filepath,sheet_name='Node_Data')
    LoadCombine = pd.DataFrame({'DL': [DL],'LL' : [LL]})
    OneWay_df = pd.DataFrame({'หมายเลขพื้น':[],'NodeI' : [] , 'NodeJ' : [] , 'NodeK' : [] , 'NodeL' : [] , 'Xlenght' : [] , 'Ylenght' : [] , 'm' : [],'Ix' : [],'Iy' : [],'Jx':[],'Jy':[],'Kx' : [],'Ky' : [],'Lx':[],'Ly':[],'DL' :[],'LL':[] })
    OneWay_Result_df = pd.DataFrame({})
    #display(Slab_Data_df)


    Interval_df = {'M-Con' : 0.0909091 ,'M-Discon' : 0,'M-Mid' :  0.0625 }
    OneDisCon_df =  {'M-Con' : 0.1,'M-Discon' : 0.0625 ,'M-Mid' :  0.07143 }
    ...
    for index, row in OneWay_df.iterrows():
        ...
        for check in range(0,5) : # ตรวจสอบความถูกต้องด้วยการวนลูป
            for i in Moment_list :
                ...
                DL = 2.4*thickness_initial
                C = Coeff_list[i]
                Moment_list[i]  = float(C) * ( (DL*(float(LoadCombine.loc[0,'DL']))) + (LL*(float(LoadCombine.loc[0,'LL']))) ) * ( min([Xlenght,Ylenght])  **2 )      
                Mmax = max(Moment_list.values())
                Vmax = ((DL*(float(LoadCombine.loc[0,'DL']))) + (LL*(float(LoadCombine.loc[0,'LL']))) * min([Xlenght,Ylenght])) * 1.15 / 2
                thickness_last = thicknessCheck(Mu = Mmax,Vu = Vmax,fc = fc,fy = fy,botcover = 0.02,thickness=thickness_initial,safetyfactor=0.9)
                thickness_temp = thickness_last
            if thickness_initial == thickness_last :
                break
        ...
        As_list = {'S' : 0 ,'V' : 0 ,'N' : 0 ,'W' : 0 ,'H' : 0 , 'E':0 }
        for i in As_list :
            As_list[i] = round(Slab_getAs(Mu = Moment_list[i]
                                        ,fc = fc,fy = fy,botcover = 0.02,thickness=thickness_use,safetyfactor=0.9),2)
        ...
        newrow_result = pd.Series({'หมายเลขพื้น' : row['หมายเลขพื้น'],'SlabType' : 'One-Way','Case' : Case ,'Xlength' : Xlenght,'Ylength' : Ylenght, 'Thickness' : f'{thickness_use:.2f}','Top Cover' : Top_Cover ,'Bot Cover' : Bot_Cover ,'AS#1':As_list['S'],'AS#2':As_list['V'],'AS#3':As_list['N'],'AS#4':As_list['W'],'AS#5':As_list['H'],
                                'AS#6':As_list['E'],'Use ST#1' : '','Use ST#2' : get_UseST(As=As_list['V'],thickness=thickness_use,Mu=Moment_list['V'],fc=fc),
                                'Use ST#3' : '','Use ST#4' : '','Use ST#5' : get_UseST(As=As_list['H'],thickness=thickness_use,Mu=Moment_list['H'],fc=fc),'Use ST#6' : ''})

        OneWay_Result_df = pd.concat([OneWay_Result_df, newrow_result.to_frame().T], ignore_index=True)
    return OneWay_Result_df

if __name__ == "__main__":
    a = TwoWay_analysed(filepath = r'C:\ProjectVenv\.venv\ProjectHouse\Ex6_1.xlsx',DL = 1.4,LL = 1.7,fc=210,fy=4000)
    b = OneWay_analysed(filepath = r'C:\ProjectVenv\.venv\ProjectHouse\Ex6_1.xlsx',DL = 1.4,LL = 1.7,fc=210,fy=4000)
    all_slab = pd.concat([a,b],ignore_index = True)
    all_slab = all_slab.sort_values(by='หมายเลขพื้น')
    save_excel_sheet(all_slab, filepath = r'C:\ProjectVenv\.venv\ProjectHouse\Ex6_1-Analysed.xlsx', sheetname='Slab_Result', index=False)
    print(all_slab)